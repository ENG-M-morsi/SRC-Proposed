    ...
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        if self.args.load != '':
            ...
            if os.path.isfile(opt_path):
                try:
                    # ============================================================
                    # ✅ حساب رقم الـ epoch الذي سنبدأ منه بناءً على طول ckp.log
                    # ckp.log يحتوي على النتائج المحفوظة حتى آخر epoch مكتمل.
                    # نبدأ من التالي: len(ckp.log) + 1
                    # ============================================================
                    resume_epoch = len(ckp.log) + 1
                    self.optimizer.load(ckp.dir, epoch=resume_epoch)
                    self.ckp.write_log(f'Optimizer state loaded. Resuming from epoch {resume_epoch}.')
                except ValueError:
                    self.ckp.write_log(
