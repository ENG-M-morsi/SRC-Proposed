...
class Loss(nn.modules.loss._Loss):
    ...
    def load(self, apath, cpu=False):
        """
        تحميل حالة الخسارة وسجل الخسارة بأمان.
        إذا كان الملف تالفاً، يتم تجاهله وبدء سجل جديد.
        """
        if cpu:
        ...
        # تحميل loss.pt بأمان
        loss_pt_path = os.path.join(apath, 'loss.pt')
        if os.path.exists(loss_pt_path):
            try:
                self.load_state_dict(torch.load(loss_pt_path, **kwargs))
                print(f'✅ Loaded loss state from {loss_pt_path}')
            except (KeyError, RuntimeError, FileNotFoundError) as e:
                print(f'⚠️ Failed to load loss.pt: {e}. Starting fresh loss.')
        else:
            print('No loss.pt found. Starting fresh loss.')

        # تحميل loss_log.pt بأمان
        loss_log_path = os.path.join(apath, 'loss_log.pt')
        if os.path.exists(loss_log_path):
            try:
                self.log = torch.load(loss_log_path, **kwargs)
                print(f'✅ Loaded loss log from {loss_log_path}')
            except (KeyError, RuntimeError, FileNotFoundError) as e:
                print(f'⚠️ Failed to load loss_log.pt: {e}. Starting new log.')
                self.log = torch.Tensor()
        else:
            print('No loss_log.pt found. Starting new log.')
            self.log = torch.Tensor()

        # تحديث جدول التعلم إذا كان السجل موجوداً
        if len(self.log) > 0:
            for l in self.get_loss_module():
                if hasattr(l, 'scheduler'):
