...
"""class P_HTCB(nn.Module):
    ...
    def forward(self, input):
        input0_1 = self.esa(input)
        input0_2 = self.esa(input0_1)
        input0 = self.esa(input0_2)
        input1 = self.swinT(input0)
        #input2 = self.esa(input)
        input2 = self.swinT(input0)
        input_esa = self.c1_r(input1) + self.c1_r(input2)
        out_fused_1 = self.esa(self.c1_r(input_esa)+input)
        out_fused_2 = self.esa(out_fused_1)
        out_fused   = self.esa(out_fused_2)
        return out_fused"""
class P_HTCB(nn.Module):
