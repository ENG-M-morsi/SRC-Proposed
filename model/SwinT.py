...
class MFEB(nn.Module):
    """
    Multi-Frequency Enhancement Block — نسخة محسّنة
    تُعلّم كيف تعزز الترددات بدلاً من قيمة ثابتة
    """
    def __init__(self, channels):
        super(MFEB, self).__init__()
        mid = max(channels // 4, 16)

        # ✅ Learnable magnitude scaling per-channel (بدلاً من mag**1.1)
        #self.freq_weight = nn.Parameter(torch.ones(1, channels, 1, 1))
        #self.freq_bias   = nn.Parameter(torch.zeros(1, channels, 1, 1))

        # ✅ Laplacian HF extraction (بدون FFT — أخف وأكثر stability)
        self.hf_branch = nn.Sequential(
            nn.Conv2d(channels, mid, 3, padding=1, groups=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid, channels, 1, bias=False),
        )

        # ✅ Gating للتحكم بمقدار التعزيز
    ...
    def forward(self, x):
        # HF عبر Laplacian Residual
        x_smooth = nn.functional.avg_pool2d(x, 3, stride=1, padding=1)
        x_hf = x - x_smooth                     # ← ترددات عالية فعلية
        x_hf = self.hf_branch(x_hf)             # ← تعلّم معالجتها

        # Gated fusion
        gate   = self.gate(x_hf)          # ✅ إصلاح: Gate على x_hf
        x_freq  = self.conv_fuse(torch.cat([x, x_hf], dim=1))
        out     = x + gate * x_freq             # Fidelity-preserving

        return out
class SwinT(nn.Module):
    def __init__(self, n_feats=93):
        ...
        self.transformer_body = BasicLayer(
            dim=n_feats,
            depth=depth,
        ...
        # ✅ MFEB inserted AFTER STL
        self.mfeb = MFEB(n_feats)
